...
static int get_loongson_fprev(struct cpuinfo_mips *c)
{
...
{
	unsigned long ases = c->ases;
	unsigned long long options = c->options;
	int data = c->loongson_cpucfg_data[0];
...
static void patch_cpucfg_sel2(struct cpuinfo_mips *c)
{
	unsigned long ases = c->ases;
	unsigned long long options = c->options;
	int data = c->loongson_cpucfg_data[1];
	...
	c->loongson_cpucfg_data[1] = data;
}

static void patch_cpucfg_sel3(struct cpuinfo_mips *c)
{
	unsigned long ases = c->ases;
	int data = c->loongson_cpucfg_data[2];
