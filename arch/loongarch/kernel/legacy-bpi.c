...
{
	int i;
	u64 mem_start, mem_end, mem_size, old_mem_start, old_mem_size;
	...
	for (i = 0; i < bpi_memmap->map_count; i++) {
		...
		case ADDRESS_TYPE_RESERVED:
			/*
			 * conversion from EFI to BPI type is lossy, so we can
			 * only approximate and treat all reserved regions as
			 * EFI runtime services related
			 */
			synth_efi_memmaps[i].type = EFI_RUNTIME_SERVICES_DATA;
			break;
		...
		old_mem_start = mem_start = bpi_memmap->map[i].mem_start;
		old_mem_size = mem_size = bpi_memmap->map[i].mem_size;
		mem_end = mem_start + mem_size;
		switch (bpi_memmap->map[i].mem_type) {
		case ADDRESS_TYPE_SYSRAM:
		case ADDRESS_TYPE_ACPI:
			/* seems to be necessary on BPI systems */
		...
		synth_efi_memmaps[i].phys_addr = mem_start;
		synth_efi_memmaps[i].virt_addr = TO_CACHE(mem_start);
		synth_efi_memmaps[i].num_pages = mem_size >> EFI_PAGE_SHIFT;
		synth_efi_memmaps[i].attribute = 0;

		if (old_mem_start != mem_start || old_mem_size != mem_size)
			pr_info(PREFIX
			        "         aligned to 0x%016llx size=0x%llx\n",
			        mem_start, mem_size);
	}
